...
<script>
    ...
    function populateServerForSched(appCode, schedName){
            ...
            "success": function (response) {
                if (response != null && '0000' == response.resultCode) {
                    ...
                    for(var index in serverList){
                        serverDiv = serverDiv + '<tr><td>'+schedName+'</td><td>'+serverList[index].ip+'</td><td>'+serverList[index].port+'</td><td></td></tr>';
                    }
                    if(serverDiv != ''){
